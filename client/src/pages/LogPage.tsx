...
export default function LogPage() {
  ...
  const [lastEntry, setLastEntry] = useState<{ exercise: string; amount: number } | null>(null);
  ...
  useEffect(() => {
    ...
    } else if (savedType && ["push-up", "pull-up", "dip", "run"].includes(savedType)) {
      setType(savedType as any);
    }
  }, []);

  const handleTypeChange = (newType: "push-up" | "pull-up" | "dip" | "run" | string) => {
    if (["push-up", "pull-up", "dip", "run"].includes(newType as string)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Use manual input if custom field is active, otherwise use slider/button amount
    let finalAmount = amount;
    
    if (customInputActive && manualInputValue.trim()) {
      const parsed = parseFloat(manualInputValue);
    ...
    try {
      ...
      // Store last entry for revert
      setLastEntry({ exercise: exerciseName, amount: finalAmount });
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-4">
                  <Button 
                    ...
                    disabled={(amount <= 0 && !(customInputActive && manualInputValue.trim())) || isSubmitting}
                  >
              ...
              <Button
                ...
                onClick={() => {
                  toast({
                    ...
                    description: `Removed ${lastEntry?.amount} reps of ${lastEntry?.exercise}.`,
                  });
                  setLastEntry(null);
